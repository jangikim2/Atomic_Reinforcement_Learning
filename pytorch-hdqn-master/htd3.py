        ...
        for i_episode in range(1000):
            ...
            while not done:
                ...
                while not done and not goal_reached:
                    ...
                    agent.ctrl_replay_memory.add(joint_state_goal, joint_next_state_goal, action, intrinsic_reward, done)
                    # Update Both meta-controller and controller
                    #agent.update_meta_controller(gamma)
                    #agent.update_controller(gamma)

                    agent.update_meta_controller(gamma=gamma, episode_timesteps=episode_length, args=args)
                    agent.update_controller(gamma=gamma, episode_timesteps=episode_length, args=args)
                ...
                agent.meta_replay_memory.add(encoded_current_state, encoded_next_state, goal, total_extrinsic_reward, done)
