...
class RolloutWorker:
    ...
    def generate_rollouts(self):
        ...
        #low_nn_at = []
        #low_nn_at = np.empty((self.high_level_train_step, self.dims['u']), np.float32)
        low_nn_at = np.empty((self.high_level_train_step*self.rollout_batch_size, self.dims['u']),
                                  np.float32).reshape(self.high_level_train_step, self.rollout_batch_size)
        intrinsic_reward = []
        ##########################################################
        ...
        for t in range(self.T):
            ...
            for i in range(self.rollout_batch_size):
                ...
                if total_timestep % self.high_level_train_step == 0:
                    ...
                    high_goal_gt_tilda[i] = self.policy.get_high_goal_gt_tilda(high_old_obj_st[i], ag[i], self.g[i], o_new[i],
                                                                            low_nn_at[i])
                    self.policy.update_meta_controller(o[i], ag[i], self.g[i], o_new[i],
                                                       np.array(high_goal_gt_tilda[i].copy()), Rt_high_sum[i], done_new[i],
                ...
                Rt_high_sum[i] += reward
                low_nn_at[i][t % self.high_level_train_step] = u.copy()
                intrinsic_reward = -LA.norm(o + high_goal_gt - o_new)

                print("cont t : ", t)
                print("cont total_timestep : ", total_timestep)

                self.policy.update_controller(o[i], o_new[i], high_goal_gt[i], u[0], intrinsic_reward[i], done_new[i].copy(),
                                              total_timestep)
                ########################################################################################################
